...
const SRC = path.resolve("Assets/projects/footwear/eclat-95");
const OUT = path.resolve("public/projects/footwear");
const CANVAS_W = 1600;
const CANVAS_H = 1200;
const MAP = [
  ["01-lateral.png", "eclat-95-01.webp"],
  ["02-perspectiva.png", "eclat-95-02.webp"],
  ["03-top.png", "eclat-95-03.webp"],
  ["04-sola.png", "eclat-95-04.webp"],
];
...
async function processFile(inputPath, outputPath, background) {
  ...
  const product = await sharp(inputPath).rotate().resize({ width: targetMax, height: targetMax, fit: "inside", withoutEnlargement: false }).sharpen({ sigma: 0.5 }).modulate({ brightness: 1.04, saturation: 1.06 }).toBuffer();
